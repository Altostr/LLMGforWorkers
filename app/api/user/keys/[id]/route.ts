...
import { z } from "zod";
import { gatewayDb, type SqlParam } from "@/lib/db";
...
export async function PUT(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const existing = await gatewayDb.get(
    "SELECT * FROM keys WHERE id = ? AND user_id = ? AND deleted_at IS NULL",
    id,
    guard.auth.user.id,
  );
  ...
  params.push(id);
  await gatewayDb.run(`UPDATE keys SET ${sets.join(", ")} WHERE id = ?`, ...params);

  const row = await gatewayDb.get("SELECT * FROM keys WHERE id = ? AND deleted_at IS NULL", id);
