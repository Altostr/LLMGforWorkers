...
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const row = await gatewayDb.get<Record<string, unknown> & { allowed_model_aliases: string }>(
...
export async function PUT(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (parsed.data.name !== undefined && parsed.data.name !== existing.name) {
    const dup = await gatewayDb.get<{ id: number }>(
      "SELECT id FROM groups WHERE name = ? AND id != ? AND deleted_at IS NULL",
      parsed.data.name,
      id,
    );
  ...
  if (setDefault) {
    await gatewayDb.run("UPDATE groups SET is_default = 0 WHERE is_default = 1");
  }
  ...
  const row = await gatewayDb.get<Record<string, unknown> & { allowed_model_aliases: string }>(
    `SELECT g.*, (SELECT COUNT(*) FROM users u WHERE u.group_id = g.id AND u.deleted_at IS NULL) AS user_count
       ...
       WHERE g.id = ?`,
    id,
  ) as Record<string, unknown> & { allowed_model_aliases: string };
...
export async function DELETE(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const group = await gatewayDb.get<{ id: number; is_default: number }>(
    "SELECT id, is_default FROM groups WHERE id = ? AND deleted_at IS NULL",
    id,
  );
  ...
  const userCount = await gatewayDb.get<{ count: number }>(
  ...
  await gatewayDb.run("UPDATE groups SET enabled = 0, deleted_at = CURRENT_TIMESTAMP WHERE id = ?", id);
