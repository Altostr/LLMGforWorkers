...
export async function GET(request: Request) {
  ...
  const total = await gatewayDb.get<{ total: number }>("SELECT COUNT(*) AS total FROM groups WHERE deleted_at IS NULL") as { total: number };
...
export async function POST(request: Request) {
  ...
  const existing = await gatewayDb.get<{ id: number }>(
    "SELECT id FROM groups WHERE name = ? AND deleted_at IS NULL",
    parsed.data.name,
  );
