...
import type { DbBatchStatement, DbRunResult, GatewayDbAdapter, SqlParam } from "@/lib/db";
...
`);
  ...
  ensureColumn("logs", "log_event_id", "log_event_id TEXT");
  ...
  ensureColumn("logs", "token_source", "token_source TEXT");
  ...
  db.exec("CREATE UNIQUE INDEX IF NOT EXISTS idx_logs_log_event_id ON logs(log_event_id) WHERE log_event_id IS NOT NULL");
...
function normalizeRunResult(result: BetterSqlite3.RunResult): DbRunResult {
  return {
...
export function createLocalGatewayDb(): GatewayDbAdapter {
  ...
  return {
    ...
    async run(sql: string, ...params: SqlParam[]) {
      return normalizeRunResult(db.prepare(sql).run(...params));
    },
    ...
    async batch(statements: DbBatchStatement[]) {
      ...
      const tx = db.transaction(() =>
        statements.map((statement) =>
          normalizeRunResult(db.prepare(statement.sql).run(...(statement.params ?? []))),
        ),
      );
      return tx();
    },
