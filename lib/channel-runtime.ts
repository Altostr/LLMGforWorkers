...
function getEffectiveLimit(state: ChannelRuntimeState, circuitBreakerEnabled = true) {
  if (!circuitBreakerEnabled) return state.maxConcurrency;
  const now = Date.now();
...
function grantLease(channelId: number, state: ChannelRuntimeState, queued: boolean, options?: RuntimeOptions) {
  state.inFlight += 1;
  return {
    ok: true as const,
    lease: new ChannelLease(channelId, state, options?.circuitBreakerEnabled ?? true),
    queued,
  };
}
...
function drainQueue(channelId: number, state: ChannelRuntimeState) {
  while (state.queue.length > 0) {
    ...
    if (!next) break;
    if (state.inFlight >= getEffectiveLimit(state, next.circuitBreakerEnabled)) break;
    ...
    waiter.resolve(grantLease(channelId, state, true, {
      circuitBreakerEnabled: waiter.circuitBreakerEnabled,
    }).lease);
  }
}
...
export class ChannelLease {
  ...
  complete(result: { ok: boolean; latencyMs: number }) {
    ...
    } else if (this.circuitBreakerEnabled) {
...
export function tryAcquireChannel(channelId: number, maxConcurrency: number, options?: RuntimeOptions): LeaseResult {
  const state = getState(channelId);
  syncStateConfig(state, maxConcurrency);
  const now = Date.now();
  const circuitBreakerEnabled = options?.circuitBreakerEnabled ?? true;

  if (circuitBreakerEnabled && state.circuitOpenUntil > now) {
    return { ok: false, reason: "circuit_open" };
  }

  if (state.inFlight >= getEffectiveLimit(state, circuitBreakerEnabled)) {
    return { ok: false, reason: "circuit_open" };
  }

  return grantLease(channelId, state, false, { circuitBreakerEnabled });
}
...
export function scoreChannel(channelId: number, staticWeight: number, maxConcurrency: number, options?: RuntimeOptions) {
  const state = getState(channelId);
  syncStateConfig(state, maxConcurrency);
  const now = Date.now();
  const circuitBreakerEnabled = options?.circuitBreakerEnabled ?? true;
