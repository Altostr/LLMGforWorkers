...
export function listEnabledAliases() {
  return gatewayDb.all<{ alias: string }>(
...
export async function listModelRoutes(
  alias: string,
  options?: { excludeChannelIds?: number[]; circuitBreakerEnabled?: boolean },
): Promise<RoutedModel[]> {
  ...
  const findRows = (targetAlias: string) => gatewayDb.all<CandidateRow>(sql, targetAlias);

  const exactRows = (await findRows(alias)).filter((row) => !exclude.has(row.channel_id_2));
  const candidateRows = exactRows.length > 0
    ? exactRows
    : (await findRows("*")).filter((row) => !exclude.has(row.channel_id_2));
...
export async function selectModelRoute(
  ...
  return (await listModelRoutes(alias, options))[0] ?? null;
}
