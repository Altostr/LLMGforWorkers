...
function resolveSecret(envKey: string, globalKey: "__jwtAccessSecret__" | "__jwtRefreshSecret__"): string {
  const configured = getRuntimeEnvValue(envKey);
...
function getAccessSecret() {
  return resolveSecret("JWT_ACCESS_SECRET", "__jwtAccessSecret__");
}

function getRefreshSecret() {
  return resolveSecret("JWT_REFRESH_SECRET", "__jwtRefreshSecret__");
}

function getAccessExpiresSeconds() {
  return positiveSeconds(getRuntimeEnvValue("JWT_ACCESS_EXPIRES_SECONDS"), 900);
}

function getRefreshExpiresSeconds() {
  return positiveSeconds(getRuntimeEnvValue("JWT_REFRESH_EXPIRES_SECONDS"), 604800);
}
...
export async function requireWebAuth(request: Request): Promise<AuthContext | null> {
  if (isAuthDisabled()) return noAuthContext();

  try {
    const token = getAccessTokenFromRequest(request);
    if (!token) return null;
    return await getAuthContextFromAccessToken(token);
  } catch {
    return null;
  }
}
...
export async function getServerProfileFromCookieStore(cookieStore: { get: (name: string) => { value: string } | undefined }) {
  if (isAuthDisabled()) return (await noAuthContext()).user;
